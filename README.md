...
### 班表管理規則
- 同一醫師在同一日期只能有一筆班表（Schedule），但可以有多個時段（TimeSlot）
- 支援早班（09:00-12:30）、午班（14:00-17:30）、晚班（18:00-21:00）
- 每個時段預設 30 分鐘一格
- API 使用 Prisma Transaction 確保原子性操作，避免 race condition 導致班表建立但時段遺失
...
- 併發處理：資料庫層級鎖定，先到先得
- 病患識別：以身分證字號作為唯一識別
- LINE 通知：有綁定 LINE 時發送預約通知，無綁定時預約仍成功但無通知
...
| 管理員取得患者列表 | `GET /api/admin/patients` |
